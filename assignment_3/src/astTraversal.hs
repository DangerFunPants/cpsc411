module AstTraversal (traverseProg) where
...
traverseArrayDimensions (ArrayDimensions_P0 exprList) = 
    fmap mapFn exprList
    where mapFn = \e -> case e of (ArrDims_P0 _ exp _) -> traverseExpr exp
...
traverseMoreParams (MoreParameters_P0 basicDecl morePs) = 
    basicDecl'++morePs'
        where 
            basicDecl' = traverseBasicDeclaration basicDecl
            morePs' = traverseMoreParams morePs
...
traverseTypeList (TypeList_P0 t moreT) = (traverseType t):(traverseMoreType moreT)

traverseMoreType :: MoreType -> [M_type]
traverseMoreType (MoreType_P0 _ t moreT) = (traverseType t):(traverseMoreType moreT)
...
traverseVarList (VarList_P0 _ varListPrime _) = varListPrime'
        where varListPrime' = traverseVarListPrime varListPrime
...
traverseIntFactor (IntFactor_P6 (IVAL val)) = M_ival $ (read val :: Integer)
traverseIntFactor (IntFactor_P7 (RVAL val)) = M_rval $ (read val)
traverseIntFactor (IntFactor_P8 (BVAL val)) = M_bval $ (readBool val :: Bool)
    where
        readBool inp
            | inp == "true" = True
            | inp == "false" = False
...
traverseIntFactor (IntFactor_P9 _ intFact) = 
    M_app (M_sub, [traverseIntFactor intFact])
