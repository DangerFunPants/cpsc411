...
next the symbol table for f and finally for g:
    ...
    [ symbol_table(1,2,[ var_attr(a,int(int,2),-5)
                       , var_attr(b,int,-4)
                       , var_attr(y,int,1)])
    , symbol_table(1,2,[ var_attr(y,int,-5)
                       , var_attr(z,int,-4)
                       , var_attr(x,int,1)
                       , array_attr(a,2,int,2)
                       , fun_attr(g,([int,array(int,2)],int),code_label_g)])
    , symbol_table(2,0,[ var_attr(x,int,1)
                       , fun_attr(f,([int,int],int),code_label_f)
                       , var_attr(v,int,2)])
    ]
...
datatype SYM_DESC = ARGUMENT of string * M_type * int
                  | VARIABLE of string * M_type * int
...
Here there is an "exception" (which is exported).  A type of symbol table 
value (which is private): this can be either the attributes of a function 
or of a variable.  For a variable we hold its offset (which can be negative) 
and its type.  For a function we hold its label and its type.  
...
levels until we find the symbol: 
   ...
   val lookup: ST * string -> SYM_I_DESC = fn (s, x) => 
    ...
    let fun found (Level, var_attr(Offset,Type,Dim)) 
                    =  I_VARIABLE(Level,Offset,Type,Dim)
...
(which may have had dynamic array allocation).
