...
addFunctionScope :: Int -> SymbolTable ()
addFunctionScope argC = State $ \s -> (addFunctionScope' argC s)
    where
      addFunctionScope' argC (labelNo, stc) = ((), (labelNo, ((newSt argC): stc)))
      newSt argC = SymbolTable 1 (negate (3 + argC)) []
...
insertSymbolTable key arg@(Argument _ _ _) =
  ...
  where
    addArgToTable ((SymbolTable posOff negOff syms):rest) key (Argument name argT dims) =
      (SymbolTable posOff (succ negOff) ((key, VarAttr negOff argT dims) : syms)) :
      rest
...
addFunctionScopeLevel :: Int -> ExceptT String SymbolTable ()
addFunctionScopeLevel argC = do
  lift $ addFunctionScope argC
...
insertAllConstructors :: [InSymbolDescription] -> ExceptT String SymbolTable ()
insertAllConstructors cList = do
    insertAllConstructors' cList 1
        where
            insertAllConstructors' (c1@(Constructor cName _ _):rest) consNum = do
                lift $ insertSymbolTableCons cName c1 consNum
                insertAllConstructors' rest (succ consNum)
                return ()
            insertAllConstructors' [] _ = return ()
    
insertSymbolTableCons :: String -> InSymbolDescription -> Int -> SymbolTable ()
insertSymbolTableCons key cons@(Constructor _ _ _) consNum = do
  State $ \s -> ((), (fst s, (addConsToTable (snd s) key cons consNum)))
  where
    addConsToTable ((SymbolTable posOff negOff syms):rest) key (Constructor name argTypes consType) consNum =
      (SymbolTable
