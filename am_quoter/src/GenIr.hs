...
walkDecl (M_var (name, arrInd, varT)) = do
  ...
  insertSymbol name inSym
  (IVariable _ offset _ _) <- lookupSymbol name
  arrExpr <- mapM walkExpr arrInd
  if (checkArrInd arrExpr)
    then do
      let retVal = ArrDecl ((offset), (fmap fst arrExpr))
      return [retVal]
...
walkDecl (M_fun (name, argList, retT, declList, stmtList)) = do
  ...
  addFunctionScopeLevel (length argList)
...
walkDecls declList@(_:_) = do
  ...
  where
    ...
    isData _          = False
    preProcDataTypes :: [M_decl] -> ExceptT String SymbolTable ()
    preProcDataTypes ((M_data (name, argList)):rest) = do
      let inSym = (Datatype (fmap fst argList))
      insertSymbol name inSym
      let consSymList = fmap (convToConsSym name) argList
      insertAllConstructors consSymList
      preProcDataTypes rest
      return ()
    preProcDataTypes [] = return ()
    convToConsSym tName (consName, paramTs) = Constructor consName paramTs tName 
walkDecls [] = return []
...
walkExpr (M_size (ident, cardinality)) = do
  ...
  if cardinality >= (dims)
    ...
    else return $ ((ISize (level, offset, cardinality)), M_int)
  ...
  where
    getTypedOp' eList opType =
      case eList of
        ...
        ((_, M_bool):[]) -> (validType M_bool opType)
        ((_, M_int):[]) -> (validType M_int opType)
        ((_, M_real):[]) -> (validType M_real opType)
...
validIntOp (M_neg) = return (INegI, M_int)
validIntOp (op) =
...
validRealOp (M_floor) = return (IFloor, M_int)
validRealOp (M_ceil) = return (ICeil, M_int)
validRealOp (M_neg) = return (INegF, M_real)
validRealOp (op) =
...
walkStmt (M_ass (name, indExpr, expr)) = do
  ...
  arrExpr <- mapM walkExpr indExpr
  if ((t == varT) && (dims == (length indExpr)) && checkArrInd arrExpr)
    then return $ IAssign (level, offset, (fmap fst arrExpr), rhsExpr)
...
walkStmt (M_case (expr, caseList)) = do
  ...
  case exprT of
    (M_type tName) -> do
      (IType consList) <- lookupSymbol tName
      if ((length consList) /= (length caseList))
        then throwE "Must case on all constructors of a type."
        else do
